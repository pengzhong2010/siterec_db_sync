...
class PyMysql:
    ...
    def __init__(self, host, port, user, passwd, db, charset="utf8", autocommit=False):
        ...
        while self._cursor is None and i < self._retry:
            try:
                ...
                if self._conn.open:
                    pass
                    # print "mysql connected, host: %s, port: %d, user: %s, db: %s" % (self._host, self._port, self._user, self._db)
            except Exception as e:
                pass
                # print "mysql connecting error, host: %s, port: %d, user: %s, db: %s, err_msg: %s\t%s" % (self._host, self._port, self._user, self._db, str(e), traceback.format_exc().replace("\n", ""))
    ...
    def reconnect(self):
        ...
        try:
            ...
            if self._conn.open:
                pass
                # print "mysql reconnected, host: %s, port: %d, user: %s, db: %s" % (self._host, self._port, self._user, self._db)
        except Exception as e:
            time.sleep(self._internal)

            # print "mysql reconnecting error, host: %s, port: %d, user: %s, db: %s, err_msg: %s\t%s" % (self._host, self._port, self._user, self._db, str(e), traceback.format_exc().replace("\n", ""))
